...
export default function PianoCanvas({
    ...
    useEffect(() => {
        ...
        const onTouchStart = (e) => {
            ...
            if (ty >= barY - 16 && ty <= barY + 22) {
                isSeekingBar.current = true;
                return;
            }
            isScrubbing.current = true;
            scrubStartY.current = ty;
            scrubStartTime.current = currentTime();
        };
        const onTouchMove = (e) => {
            e.preventDefault();
            if (isSeekingBar.current) {
                const ratio = e.touches[0].clientX / canvas.width;
        ...
        const onMouseDown = (e) => {
            const ch = canvas.height;
            const barY = ch - KEY_H - 6;
            if (e.clientY >= barY - 16 && e.clientY <= barY + 22) {
                isSeekingBar.current = true;
                return;
            }
            isScrubbing.current = true;
        ...
        const onMouseMove = (e) => {
            if (isSeekingBar.current) {
                const ratio = e.clientX / canvas.width;
                onScrub(Math.max(0, Math.min(1, ratio)) * songDuration);
                return;
            }
            if (!isScrubbing.current) return;
    ...
    const drawProgressAndTime = useCallback((ctx, cw, ch) => {
        ...
        ctx.beginPath();
        ctx.roundRect(0, barY, cw, barH, 3);
        ctx.fill();
        // Fill
        ctx.fillStyle = '#e63946';
        ctx.beginPath();
        ctx.roundRect(0, barY, Math.max(barH, cw * prog), barH, 3);
        ctx.fill();
        // Dot
        ...
        ctx.textAlign = 'right';
        ctx.fillText(`${cur} / ${total}`, cw - 16, barY - 10);
        ctx.textAlign = 'left';
